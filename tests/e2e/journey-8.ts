...
async function gotoLocalRoot(page: Page) {
  await page.goto(`${BASE_URL}/?local=true`)
  await page.waitForLoadState('networkidle')
  await page.waitForSelector('[data-testid="app-layout"], nav, aside', {
    state: 'visible',
    timeout: 10000,
  })
}

/** 获取 simple-mind-map 根节点文本 */
async function getRootText(page: Page): Promise<string> {
  const root = await page.$('.smm-node-root .smm-node-text')
  if (!root) throw new Error('未找到根节点')
  return root.textContent() || ''
}

/** 聚焦 simple-mind-map 中的特定文字节点（支持部分匹配） */
async function focusNodeByText(page: Page, text: string) {
  const nodes = await page.locator('.smm-node-text').all()
  ...
  throw new Error(`未找到包含 "${text}" 的节点`)
}
...
export async function runJourney8(page: Page) {
  ...
  try {
    ...
    const createBtn = page.locator('button:has-text("生成并创建")')
    await createBtn.click()

    // 等待导航 + mindmap canvas
    await page.waitForURL(/\/project\//, { timeout: 15000 })
    await page.waitForSelector('.smm-node-root', { state: 'visible', timeout: 15000 })
    ...
    const map = await page.evaluate(() => {
      // @ts-expect-error exposed by simple-mind-map
      const mindmap = (window as any).__mindflow_mindmap__
      return mindmap?._data
    })
    const children = (map?.children || []) as Array<Record<string, unknown>>
    const texts = children.map((c: Record<string, unknown>) => (c.data as Record<string, unknown>)?.text as string)
    expect(texts).toContain('需求分析')
    expect(texts).toContain('设计阶段')
    expect(texts).toContain('开发实现')
    expect(texts).toContain('测试上线')
  ...
  try {
    ...
    const board = await page.$('[data-testid="project-board"]')
    expect(board).not.toBeNull()

    const cards = await page.locator('[data-testid="project-board"] [data-testid="task-card"]').all()
  ...
  try {
    ...
    await page.dblclick('.smm-node.active .smm-node-text')
